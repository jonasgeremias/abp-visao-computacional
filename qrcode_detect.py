...
EXT_MODELS = '*.obj'
MIN_MATCHES = 130
CAMERA_PARAMETERS = np.array([[500, 0, 320], [0, 500, 240], [0, 0, 1]]) # Camera parameters
...
banco_de_dados = [
    ...
    #     'qr_code_2_satc.png', 'model': PATH_MODELS + 'fox.obj', 'proportion_3D' : 5}
    # ,
    # {'id': 3, 'link': 'teste.com', 'src': PATH_QRCODES +
    #     'qr_code_3_satc.png', 'model': PATH_MODELS + 'house.obj', 'proportion_3D' : 5}
]
  ...
  for item in banco_de_dados:
    try:
      ...
      if len(good) > MIN_MATCHES:
          ...
          sourcePoints = np.float32([item['referenceImagePts'][m.queryIdx].pt for m in good]).reshape(-1, 1, 2)
          destinationPoints = np.float32([sourceImagePts[m.trainIdx].pt for m in good ]).reshape(-1, 1, 2)
